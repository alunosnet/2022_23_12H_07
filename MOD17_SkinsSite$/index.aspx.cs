...
namespace MOD17_SkinsSite_
{
    public partial class index : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //atualizar grelha Skins
            atualizaListaSkins(null, ordenar);
        }

        private void atualizaListaSkins(string pesquisa = null, int? ordena = null)
        {
            skin skin = new skin();
            DataTable dados;
            if (pesquisa == null)
            {
                //se existir o cookie ultimaskin listar as skins do mesmo autor
                HttpCookie httpCookie = Request.Cookies["ultimaskin"];
                if (httpCookie == null)
                    dados = skin.listarskinDisponiveis(ordena);
                else
                {
                    dados = skin.listaSkinDisponiveis(Server.UrlDecode(httpCookie.Value));
                    dados.Merge(skin.listarskinDisponiveis(ordena));
                }
            }
            else
            {
                dados = skin.listaSkinDisponiveis(pesquisa, ordena);
            }
            gerarIndex(dados);
        }
        ...
        private void gerarIndex(DataTable dados)
        {
            ...
            foreach (DataRow skin in dados.Rows)
            {
                grelha += "<div class='col'>";
                grelha += "<img src='/Public/Imagens/" + skin[0].ToString() +
                    ".jpg' class='img-fluid'/>";
                grelha += "<p/><span class='stat-title'>" + skin[1].ToString()
                    + "</span>";
                grelha += "<span class='stat-title'>" +
                    String.Format(" | {0:C}", Decimal.Parse(skin["preco"].ToString()))
                    + "</span>";
                grelha += "<br/><a href='detalhesSkin.aspx?id=" + skin[0].ToString()
                    + "'>Detalhes</a>";
                grelha += "</div>";
            }
            grelha += "</div></div>";
            divSkins.InnerHtml = grelha;
        }
        ...
        protected void bt_login_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Session["id"] = dados.Rows[0]["nutilizador"].ToString();
                //autorização
        ...
        {
            atualizaListaSkins(tbPesquisa.Text);
        }
